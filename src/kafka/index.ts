...
if (!KAFKA_BOOTSTRAP_URL || !KAFKA_SVC_USERNAME || !KAFKA_SVC_PASSWORD) {
  log.warn('Kafka senders set to no-op. Set KAFKA_BOOTSTRAP_URL, KAFKA_SVC_USERNAME, KAFKA_SVC_PASSWORD to send data to Kafka')
} else {
  kafkaOpts = {
  ...
  producer.on(producer.events.DISCONNECT, (e) => {
    log.error(
      `Kafka producer disconnected from broker ${KAFKA_BOOTSTRAP_URL}`
    );
    log.error(e);
    process.exit(1);
  });
