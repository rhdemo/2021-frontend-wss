...
type DataGridEventHandle = (client: Client, event: ClientEvent, key: string) => void

async function getClient(nodes: InfinispanNode[], cacheName: string): Promise<Client> {
...
export default async function getDataGridClientForCacheNamed (cacheName: string, eventHandler: DataGridEventHandle): Promise<Client> {
  ...
  const listenerId = await client.addListener(
    'create',
    (key, version, id) => eventHandler(client, 'create', key)
  );

  await client.addListener('modify', key => eventHandler(client, 'modify', key), { listenerId });
  await client.addListener('remove', key => eventHandler(client, 'remove', key), { listenerId });
