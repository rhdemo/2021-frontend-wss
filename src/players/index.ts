...
export async function initialise () {
  dg = await getDataGridClientForCacheNamed(DATAGRID_PLAYER_DATA_KEY, playerDataGridEventHandler)
}
...
export async function getPlayerByUsername (username: string) {
  ...
  if (data) {
    try {
      ...
      log.warn(`found player data for "${username}", but failed to parse to JSON: %j`, data)
      return undefined
    }
  } else {
    return undefined
